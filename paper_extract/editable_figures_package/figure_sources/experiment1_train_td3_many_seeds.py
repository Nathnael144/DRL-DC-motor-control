...
from dc_motor_env import (
    DCMotorSpeedEnv,
    MotorParams,
    ResidualHinfDCMotorEnv,
    ResidualLQRDCMotorEnv,
    ResidualMPCDCMotorEnv,
    SCENARIO_NAMES,
...
def make_motor_env(
    ...
    **kwargs: Any,
    ...
    if control_mode == "residual_mpc":
        return ResidualMPCDCMotorEnv(
            residual_voltage_limit=residual_voltage_limit,
            **kwargs,
        )
    if control_mode == "residual_hinf":
        return ResidualHinfDCMotorEnv(
            residual_voltage_limit=residual_voltage_limit,
            **kwargs,
        )
...
def expert_action_for_env(
    ...
    elif control_mode == "residual_mpc":
        baseline = float(env._mpc_voltage())  # type: ignore[attr-defined]
        normalized = (voltage - baseline) / max(residual_voltage_limit, 1e-6)
    elif control_mode == "residual_hinf":
        baseline = float(env._hinf_voltage())  # type: ignore[attr-defined]
        normalized = (voltage - baseline) / max(residual_voltage_limit, 1e-6)
    else:
        raise ValueError(f"Unknown control_mode: {control_mode}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--control-mode",
        choices=("direct", "residual_lqr", "residual_mpc", "residual_hinf"),
        default="direct",
        help="Use direct voltage control or learn a residual correction on top of nominal LQR/MPC/H-infinity.",
    )
    parser.add_argument(
        "--residual-voltage-limit",
        type=float,
        default=8.0,
        help="Maximum absolute RL correction voltage when a residual control mode is used.",
    )
    parser.add_argument(
