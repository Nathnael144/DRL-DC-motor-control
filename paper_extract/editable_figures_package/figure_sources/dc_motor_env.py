...
def _hinf_state_feedback_gain(
    ...
    hamiltonian = np.block([[a, m], [-q, -a.T]])
    eigvals, eigvecs = np.linalg.eig(hamiltonian)
    stable = np.where(np.real(eigvals) < 0.0)[0]
    if stable.size != a.shape[0]:
        stable = np.argsort(np.real(eigvals))[: a.shape[0]]
    ...
    p = np.real(v2 @ np.linalg.inv(v1))
    p = 0.5 * (p + p.T)
    return np.real(r_inv @ b2.T @ p)


def _rk4_linear_discretization(
    a: np.ndarray,
    b: np.ndarray,
    dt: float,
    substeps: int = 10,
...
def _steady_state_for_reference(
    a: np.ndarray,
    b: np.ndarray,
    c: np.ndarray,
    reference: float,
    ...
    rhs = np.array([0.0, 0.0, reference], dtype=np.float64)
...
class ResidualMPCDCMotorEnv(DCMotorSpeedEnv):
    """Environment where the agent learns a residual voltage on top of nominal MPC."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.residual_voltage_limit = float(residual_voltage_limit)
        a, b, c = _nominal_motor_state_space(self.params)
        ad, bd = _rk4_linear_discretization(a, b, dt=self.control_dt, substeps=10)
        q = np.diag([0.004, 4.5])
        r = np.array([[0.075]], dtype=np.float64)
        self._mpc_a = a
        self._mpc_b = b
        self._mpc_c = c
        self._mpc_k0 = _dlqr_gain(ad, bd, q, r)

    def _mpc_voltage(self) -> float:
        x = np.asarray(self.state, dtype=np.float64)
        reference = self._reference(self.t)
        x_ss, u_ss = _steady_state_for_reference(self._mpc_a, self._mpc_b, self._mpc_c, reference)
        return float(u_ss - (self._mpc_k0 @ (x - x_ss).reshape(-1, 1)).item())

    def step(self, action: np.ndarray):
        residual_normalized = float(np.clip(action[0], -1.0, 1.0))
        baseline_voltage = self._mpc_voltage()
        residual_voltage = residual_normalized * self.residual_voltage_limit
        combined_voltage = float(
            np.clip(
                baseline_voltage + residual_voltage,
...
class ResidualHinfDCMotorEnv(DCMotorSpeedEnv):
    """Environment where the agent learns a residual voltage on top of H-infinity feedback."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.residual_voltage_limit = float(residual_voltage_limit)
        a, b, c = _nominal_motor_state_space(self.params)
        b1 = np.array([[0.0], [-1.0 / self.params.j]], dtype=np.float64)
        q = np.diag([0.001, 7.0])
        r = np.array([[0.045]], dtype=np.float64)
        gamma = 50.0
        self._hinf_a = a
        self._hinf_b = b
        self._hinf_c = c
        self._hinf_k = _hinf_state_feedback_gain(a, b, b1, q, r, gamma)
        denom = c @ np.linalg.inv(a - b @ self._hinf_k) @ b
        self._hinf_nbar = float(-1.0 / denom.item())

    def _hinf_voltage(self) -> float:
        x = np.asarray(self.state, dtype=np.float64)
        reference = self._reference(self.t)
        feedback = float((-self._hinf_k @ x.reshape(-1, 1)).item())
        return feedback + self._hinf_nbar * reference

    def step(self, action: np.ndarray):
        residual_normalized = float(np.clip(action[0], -1.0, 1.0))
        baseline_voltage = self._hinf_voltage()
        residual_voltage = residual_normalized * self.residual_voltage_limit
        combined_voltage = float(
            np.clip(
                baseline_voltage + residual_voltage,
