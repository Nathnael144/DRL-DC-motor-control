...
def hinf_state_feedback_gain(
    ...
    """Solve a small continuous-time H-infinity state-feedback Riccati equation."""
    r_inv = np.linalg.inv(r)
    m = (b1 @ b1.T) / (gamma**2) - b2 @ r_inv @ b2.T
    h = np.block(
        [
            [a, m],
            [-q, -a.T],
    ...
    v1 = v[: a.shape[0], :]
    v2 = v[a.shape[0] :, :]
    if abs(np.linalg.det(v1)) < 1e-12:
        raise ValueError("H-infinity Riccati stable subspace is ill-conditioned.")
    p = np.real(v2 @ np.linalg.inv(v1))
    p = 0.5 * (p + p.T)
    if np.any(np.linalg.eigvalsh(p) < -1e-7):
        raise ValueError("H-infinity Riccati solution is not positive semidefinite.")
    k = r_inv @ b2.T @ p
    return np.real(k), p
...
class HInfinityStateFeedbackController(BaseController):
    name = "Hinf_state_feedback"

    def __init__(self, params: MotorParams, q_current: float, q_speed: float, r_voltage: float, gamma: float):
        self.a, self.b, self.c = motor_state_space(params)
        b1 = np.array([[0.0], [-1.0 / params.j]], dtype=float)
        q = np.diag([q_current, q_speed])
        r = np.array([[r_voltage]], dtype=float)
        self.k, _ = hinf_state_feedback_gain(self.a, self.b, b1, q, r, gamma)
        denom = self.c @ np.linalg.inv(self.a - self.b @ self.k) @ self.b
        self.nbar = float(-1.0 / denom.item())
...
def tune_hinf_on_nominal_and_stress() -> tuple[float, float, float, float]:
    """Small fixed-design sweep; the selected H-infinity gain is then frozen."""
    params = MotorParams()
    q_current_values = [0.001, 0.003]
    q_speed_values = [2.8, 4.5, 7.0]
    r_values = [0.045, 0.065, 0.085]
    gamma_values = [50.0, 100.0, 200.0, 500.0, 1000.0]
    tune_cases = [
        ("nominal", "step_nominal"),
        ("nominal", "ramp"),
        ("ra_plus_50", "step_load_disturbance"),
        ("saturation", "ramp"),
        ("combined_stress", "step_load_disturbance"),
    ]
    best_score = float("inf")
    best = (0.003, 4.5, 0.065, 1.2)

    for q_current in q_current_values:
        for q_speed in q_speed_values:
            for r_voltage in r_values:
                for gamma in gamma_values:
                    try:
                        controller = HInfinityStateFeedbackController(params, q_current, q_speed, r_voltage, gamma)
                    except (ValueError, np.linalg.LinAlgError, FloatingPointError):
                        continue
                    score = 0.0
                    valid = True
                    for condition_name, scenario_name in tune_cases:
                        row = rollout_controller(controller, condition_name, scenario_name, seed=321)
                        if not np.isfinite(row["iae"]):
                            valid = False
                            break
                        score += row["iae"] + 0.05 * row["steady_state_error"] + 0.0005 * row["control_energy"]
                    if valid and score < best_score:
                        best_score = score
                        best = (q_current, q_speed, r_voltage, gamma)
    return best
...
def build_classical_controllers() -> list[Callable[[], BaseController]]:
    ...
    hinf_q_current, hinf_q_speed, hinf_r, hinf_gamma = tune_hinf_on_nominal_and_stress()
    return [
        lambda: PIDController(kp, ki, kd),
        lambda: LQRController(params),
        lambda: LQIController(params),
        lambda: HInfinityStateFeedbackController(params, hinf_q_current, hinf_q_speed, hinf_r, hinf_gamma),
